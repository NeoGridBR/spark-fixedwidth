...
import com.quartethealth.spark.csv.readers.{BulkCsvReader, BulkReader, LineCsvReader, LineReader}
import com.quartethealth.spark.csv.util._
      ...
      if (dropMalformed && schemaFields.length != tokens.size) {
        ...
        try {
          ...
          while (index < schemaFields.length) {
            ...
            val formattedToken = MetadataFields.formatTokenBasedOnMetadata(tokens(index), field)
            rowArray(index) = TypeCast.castTo(formattedToken, field.dataType, field.nullable,
